...
> **상태: 구현함 (2026-09-21).** 미결정 사항은 다음처럼 정했다. 1) 라이브러리 없이 CSS 막대. 2) 7일 / 30일만. 3) 관리자 버튼은 통계 페이지로 연결하고, Cloudflare 대시보드 링크는 통계 페이지 안에 "Cloudflare에서 자세히 보기"로 남김. 캐시는 권한 확인 뒤 기간별 통계 결과만 5분 캐시한다(모든 허용 사용자에게 같은 데이터라 사용자별 키가 필요 없음).
