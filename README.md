...
- **빠른 추가 ▾ → 일간회고 / 월간회고**를 누르면 제목·카테고리(회고)·태그·본문 양식이 채워진 새 글이 열립니다. 열 때 페이지가 한 번 새로 고쳐집니다.
  - 일간회고 제목은 오늘 날짜(`2026/09/21 일간회고`), 월간회고 제목은 **지난달**(`2026/08 월간회고`)로 채워지고, 수정할 수 있습니다. 폴더명은 제목을 따릅니다(`2026-09-21-일간회고`).
  - 왼쪽 컬렉션의 "일간회고", "월간회고"에서 회고 글만 모아 볼 수 있습니다(전체 "글" 목록에도 그대로 보임).
  - 양식을 바꾸려면 저장소 루트의 [일간회고_양식.md](일간회고_양식.md) / [월간회고_양식.md](월간회고_양식.md) 본문과 [static/admin/config.yml](static/admin/config.yml)의 본문 기본값을 **함께** 고치세요. 둘이 다르면 `npm test`가 알려줍니다.
- **같은 날짜의 일간회고나 같은 달의 월간회고가 이미 있으면 새로 만들지 말고 목록에서 기존 글을 여세요.** 같은 제목으로 저장하면 기존 글을 덮어쓰지는 않지만, 기존 폴더 안에 `index-1.md`로 저장되어 사이트에 나오지 않습니다. (이름이 겹칠 때의 Decap 기본 동작이라 일반 "글"도 마찬가지입니다.)
