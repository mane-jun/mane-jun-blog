...
**빠른 추가 ▾ → 개발글**을 누르면 카테고리가 "개발"로 정해지고, 본문에 한 줄 요약 · 배경 · 문제 · 원인 · 해결 · 배운 점 · 참고 자료 순의 양식이 채워진 새 글이 열립니다. 제목은 직접 쓰고, 폴더명은 일반 글처럼 `날짜-제목`입니다. 쓰지 않는 칸은 지우면 됩니다.

- 왼쪽 컬렉션의 "개발글"에서 카테고리가 "개발"인 글만 모아 볼 수 있습니다(예전 개발 글 포함).
- 양식을 바꾸려면 [개발글_양식.md](개발글_양식.md) 본문과 [static/admin/config.yml](static/admin/config.yml)의 `dev` 컬렉션 본문 기본값을 **함께** 고치세요. 둘이 다르면 `npm test`가 알려줍니다.
...
사이트 상단 메뉴 **회고**(`/retro/`)에서 회고를 한눈에 봅니다.

- **잔디:** 최근 1년의 일간회고를 날짜별 칸으로 보여줍니다. 쓴 날은 초록색이고 컨디션이 좋을수록 진합니다. 칸에 마우스를 올리면 날짜·컨디션·운동이 보이고, 누르면 그날 회고로 이동합니다.
- **숫자:** 연속 기록, 이번 달 쓴 날, 이번 달 평균 컨디션, 일간·주간·월간 회고 수. "오늘"은 보는 사람 브라우저의 한국 시간 기준입니다.
- **목록:** 주간회고·월간회고 링크.
- 날짜는 **제목**(`2026/09/21 일간회고`)에서 읽으므로, 자정이 지나 전날 회고를 써도 제목 날짜에 표시됩니다. 컨디션·운동은 본문의 `컨디션: 3/5`, `운동: O`(또는 `X`) 줄에서 읽습니다. 양식 그대로 `/5` 앞에 숫자만 채우면 되고, 비워 두면 중간 색으로 표시됩니다.
- 관련 파일: [content/retro.md](content/retro.md), [layouts/retro.html](layouts/retro.html), 모양은 [_custom.scss](assets/css/_custom.scss).
...
- 테마는 다크/라이트 모드를 바꿀 때 댓글 반응(이모지) 버튼을 꺼버리는 버그가 있어서, [giscus-reactions-fix.html](layouts/_partials/head/giscus-reactions-fix.html)이 모드 전환 직후 다시 켭니다.
- 댓글은 배포된 사이트(프로덕션)에서만 보입니다. 로컬에서 확인하려면 `hugo server --environment production`으로 띄우세요.
...
CMS 설정과 개발 글·회고 양식이 맞는지, 이미지만 남은 폴더가 없는지, 로그인 Worker가 제대로 동작하는지 확인합니다. CMS 설정이나 Worker 코드를 고친 뒤에 실행하세요.
