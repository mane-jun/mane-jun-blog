...
CMS 설정과 글 양식이 맞는지, 이미지만 남은 폴더가 없는지, 로그인·통계 Worker가 제대로 동작하는지(권한 없는 계정 차단, 비밀값 비노출 포함) 확인합니다. CMS 설정이나 Worker 코드를 고친 뒤에 실행하세요.
...
├── static/
│   ├── admin/                     # 관리자 (Decap CMS) 페이지·설정, 통계 페이지
