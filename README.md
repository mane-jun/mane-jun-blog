...
CMS 설정과 글 양식·미리보기가 맞는지, 회고 알림이 회고를 제대로 찾는지, 예약 발행 판단이 맞는지, 이미지만 남은 폴더가 없는지, 로그인·통계 Worker가 제대로 동작하는지(권한 없는 계정 차단, 비밀값 비노출 포함) 확인합니다. CMS 설정이나 Worker 코드를 고친 뒤에 실행하세요.
...
mane-jun-blog/
├── .github/workflows/
│   ├── deploy.yml                 # GitHub Pages 자동 배포 + 매시 예약 발행 확인
