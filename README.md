...
작성일을 **미래 시각**으로 정하고 **초안 스위치를 끈 채** 저장하면, 그 시각이 지난 뒤 자동으로 사이트에 올라갑니다.

- 매시 7분에 GitHub Actions가 "작성일이 지났는데 아직 사이트에 없는 글"을 확인해 있을 때만 다시 배포합니다. 그래서 정한 시각부터 **최대 1시간쯤 늦게** 올라가고, GitHub 예약 실행이 밀리면 더 늦어질 수 있습니다.
- 그 전까지는 사이트·목록·RSS 어디에도 보이지 않습니다. 초안 스위치가 켜져 있으면 시각이 지나도 올라가지 않습니다.
- 확인은 [scheduled-posts.mjs](scripts/scheduled-posts.mjs)가 실제 사이트의 사이트맵과 비교해서 하므로, 한 번 놓쳐도 다음 확인 때 올라갑니다(최근 7일 안의 글 대상).
...
- **주간·월간회고는 숫자가 미리 채워집니다.** 그 기간 일간회고에서 "회고 쓴 날", "운동한 날", 요일별(주간)·주별(월간, 1~7일·8~14일·…) 컨디션과 평균을 계산해 넣습니다. 공개된 일간회고만 세고, 방금 공개한 일간회고는 배포(1~2분)가 끝나야 들어갑니다. 데이터를 3초 안에 못 받으면 빈 양식으로 열립니다.
  - 데이터는 [회고 모아보기](#회고-모아보기) 페이지의 것을, 양식은 `config.yml`의 본문 기본값을 그대로 씁니다. 양식의 숫자 줄(`- 회고 쓴 날: /7` 등)을 바꾸면 채우기가 안 될 수 있는데, 그때는 `npm test`가 알려줍니다. 코드는 [retro-stats.js](static/admin/retro-stats.js).
...
- 매일 **밤 10시**까지 그날 일간회고가, 일요일 밤에는 이번 주 주간회고와 지난달 월간회고가 공개되지 않았으면 GitHub 이슈로 알림이 옵니다([회고 알림](#회고-알림)).
...
- 글 아래 "수정" 날짜는 Git 기록의 실제 마지막 수정일입니다(`hugo.toml`의 `enableGitInfo`). 관리자에서 저장만 해도 날짜가 바뀝니다. 한글 파일명 글도 읽을 수 있게 배포 워크플로가 `git config core.quotepath false`를 켭니다(로컬 Windows 미리보기에서는 한글 파일명 글이 작성일로 보일 수 있음).
...
- **숫자:** 연속 기록, 이번 달 쓴 날, 이번 달 평균 컨디션, 이번 달 운동한 날, 지금까지 쓴 일간·주간·월간 회고 수. "오늘"은 보는 사람 브라우저의 한국 시간 기준입니다.
- **잔디:** 최근 1년의 일간회고를 날짜별 칸으로 보여줍니다. 오른쪽 위 버튼으로 색 기준을 바꿀 수 있고, 고른 기준은 브라우저에 기억됩니다. 칸에 마우스를 올리면 날짜·컨디션·운동이 보이고, 누르면 그날 회고로 이동합니다.
  - **컨디션:** 쓴 날은 초록색이고 컨디션이 좋을수록 진합니다(컨디션을 안 적은 날은 중간 색).
  - **운동:** 운동한 날은 초록, 안 한 날은 붉은색, 운동 기록이 없는 날은 회색입니다.
- **월별 컨디션:** 최근 12개월의 평균 컨디션을 막대로 보여줍니다. 막대에 마우스를 올리면 그달 회고 쓴 날과 운동한 날이 보입니다.
- **목록:** 주간회고·월간회고 링크.
...
[retro-reminder.yml](.github/workflows/retro-reminder.yml)이 매일 22:00(KST)에 회고를 확인합니다. 관리자 **빠른 추가**가 채우는 제목(예: `2026/09/14~09/20 주간회고`)이나 그 폴더명으로 찾습니다.

| 회고 | 확인하는 때 | 확인 대상 |
| --- | --- | --- |
| 일간 | 매일 밤 | 오늘 |
| 주간 | 일요일 밤 | 이번 주(월~일) |
| 월간 | 일요일 밤 | 지난달 (첫 일요일부터 쓸 때까지 알림 유지) |

- **없으면** "📝 2026/09/21 일간회고를 아직 안 썼어요", **초안이면** "…가 아직 초안이에요" 이슈를 만들어 `mane-jun`에게 지정합니다. 이슈에 바로 쓰기/편집 링크가 있습니다.
- **GitHub 모바일 앱**을 설치하고 알림을 켜 두면 휴대폰으로 푸시가 옵니다(지정된 이슈는 기본으로 푸시 대상). 이메일 알림은 GitHub 알림 설정을 따릅니다.
- 회고를 공개해 저장하면 해당 알림 이슈가 자동으로 닫히고, 새 알림을 만들 때 같은 종류의 지난 기간 알림도 닫힙니다. 자정이 지나 전날 일간회고를 쓴 경우에는 다음 날 밤 알림 때 닫힙니다.
- GitHub의 예약 실행은 붐비는 시간에 수십 분 늦어질 수 있습니다. 저장소에 60일 동안 커밋이 없으면 예약 실행이 멈추므로, 그때는 Actions 탭에서 다시 켜세요.
- 테스트: Actions → **Retrospective reminder** → **Run workflow**에서 날짜(`2026-09-20` 또는 `20260920`)와 회고 종류를 골라 실행하면 그 날짜 22시 기준으로 확인합니다.
- 판단 로직은 [retro-reminder.mjs](scripts/retro-reminder.mjs)(`npm test`로 검사), 이슈 처리는 [retro-reminder.sh](scripts/retro-reminder.sh)입니다.
...
배포할 때 Hugo 빌드 결과의 모든 페이지에서 사이트 안쪽 링크와 이미지 주소를 확인하고, **깨진 게 있으면 배포하지 않습니다.** 이때 GitHub Actions 실행 화면에 "깨진 링크: about/index.html → ../없는-주소/"처럼 어느 페이지의 어떤 링크인지 나오고, 실패 알림 메일이 옵니다. 글을 고쳐 다시 저장하면 배포됩니다.

- 외부 사이트 링크는 상대 서버 상태에 따라 결과가 흔들려서 검사하지 않습니다.
- 로컬에서는 `npm run check:links`로 빌드와 검사를 한 번에 할 수 있습니다. 코드는 [check-links.mjs](scripts/check-links.mjs)입니다.
...
CMS 설정과 글 양식·미리보기가 맞는지, 회고 알림이 회고를 제대로 찾는지, 예약 발행 판단과 링크 검사가 맞는지, 이미지만 남은 폴더가 없는지, 로그인·통계 Worker가 제대로 동작하는지(권한 없는 계정 차단, 비밀값 비노출 포함) 확인합니다. CMS 설정이나 Worker 코드를 고친 뒤에 실행하세요.
...
mane-jun-blog/
├── .github/workflows/
│   ├── deploy.yml                 # GitHub Pages 자동 배포 + 깨진 링크 검사 + 매시 예약 발행 확인
│   └── retro-reminder.yml         # 매일 22시 회고 알림 (일요일엔 주간·월간도)
